...
# ============================================================================
# [MODIFIED] Custom install layout — everything lives under /data/hermes/
#   Data (config, keys, sessions, logs): $HERMES_HOME = /data/hermes
#   Code (git checkout + venv):          $INSTALL_DIR = /data/hermes/hermes-agent
# Hardcoded for unattended/background installs. INSTALL_DIR_EXPLICIT=true
# keeps resolve_install_layout() from overriding these values.
# ============================================================================
...
# Options
USE_VENV=true
# [MODIFIED] Skip the interactive setup wizard — API keys & settings come from
# config.txt (see load_config_txt / apply_config_txt below).
RUN_SETUP=false
SKIP_BROWSER=false
...
STAGE_NAME=""
JSON_OUTPUT=false
# [MODIFIED] Force non-interactive: prompt_yes_no() returns the configured
# default instead of reading from /dev/tty, so a background install never
...
# ============================================================================
# Config file (config.txt) support — [MODIFIED]
# ----------------------------------------------------------------------------
# The user pre-fills API keys and install choices in a config.txt file so the
# install runs fully unattended. Search order:
#   1. $HERMES_CONFIG (explicit path)
#   2. <script-dir>/config.txt
#   3. $HERMES_HOME/config.txt
# ============================================================================
CONFIG_FILE=""
CONFIG_ENV_TMP=""
...
find_config_file() {
    if [ -n "${HERMES_CONFIG:-}" ] && [ -r "$HERMES_CONFIG" ]; then
        CONFIG_FILE="$HERMES_CONFIG"
        return 0
    fi
    local script_dir
    script_dir="$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")" 2>/dev/null && pwd)"
    if [ -r "$script_dir/config.txt" ]; then
        CONFIG_FILE="$script_dir/config.txt"
        return 0
    fi
    if [ -r "${HERMES_HOME:-}/config.txt" ]; then
        CONFIG_FILE="${HERMES_HOME}/config.txt"
        return 0
    fi
    return 1
}

# Parse config.txt. Install-choice keys are consumed here; every other
# KEY=VALUE line is collected into a temp file that apply_config_txt() later
# writes into $HERMES_HOME/.env.
load_config_txt() {
    if ! find_config_file; then
        log_info "No config.txt found — using built-in defaults (AI_PROVIDER=$AI_PROVIDER, all install options on)."
        return 0
    fi
    log_info "Reading configuration from $CONFIG_FILE"
    CONFIG_ENV_TMP="$(mktemp)"
    local line key val
    while IFS= read -r line || [ -n "$line" ]; do
        ...
        case "$key" in
            ...
            MODEL)               MODEL="$val" ;;
            INSTALL_RIPGREP)     INSTALL_RIPGREP="$val" ;;
            INSTALL_FFMPEG)      INSTALL_FFMPEG="$val" ;;
            INSTALL_BUILD_TOOLS) INSTALL_BUILD_TOOLS="$val" ;;
            INSTALL_GATEWAY)     INSTALL_GATEWAY="$val" ;;
            *)
                # Everything else (API keys, messaging tokens, ...) → .env
                [ -n "$val" ] && printf '%s\n' "$line" >> "$CONFIG_ENV_TMP"
                ;;
        esac
    done < "$CONFIG_FILE"
    log_info "  AI_PROVIDER=$AI_PROVIDER  MODEL=${MODEL:-<hermes default>}"
    log_info "  INSTALL_RIPGREP=$INSTALL_RIPGREP INSTALL_FFMPEG=$INSTALL_FFMPEG INSTALL_BUILD_TOOLS=$INSTALL_BUILD_TOOLS INSTALL_GATEWAY=$INSTALL_GATEWAY"
}

# Set (or create) KEY=VALUE inside $HERMES_HOME/.env.
set_env_var() {
    local key="$1" value="$2"
    [ -z "$key" ] && return 0
    if grep -q "^${key}=" "$HERMES_HOME/.env" 2>/dev/null; then
        sed -i "s|^${key}=.*|${key}=${value}|" "$HERMES_HOME/.env"
    else
        printf '%s=%s\n' "$key" "$value" >> "$HERMES_HOME/.env"
    fi
}

# Write the API keys / settings from config.txt into $HERMES_HOME/.env.
apply_config_txt() {
    if [ -z "$CONFIG_ENV_TMP" ] || [ ! -s "$CONFIG_ENV_TMP" ]; then
        return 0
    fi
    log_info "Applying API keys / settings from $CONFIG_FILE → $HERMES_HOME/.env"
    local line key val
    while IFS= read -r line || [ -n "$line" ]; do
        [ -z "$line" ] && continue
    ...
    log_success "API keys / settings applied to $HERMES_HOME/.env"
}

# Configure model.provider (+ optional model.default) in config.yaml so the
# chosen provider is used by default, without running the interactive wizard.
# Uses a targeted sed scoped to the `model:` YAML section (comment-preserving).
apply_provider_config() {
    [ -n "$AI_PROVIDER" ] || AI_PROVIDER="openrouter"
    local cfg="$HERMES_HOME/config.yaml"
    [ -f "$cfg" ] || return 0

    local safe_provider safe_model
    safe_provider="$(printf '%s' "$AI_PROVIDER" | tr -cd 'a-zA-Z0-9_-')"
    sed -i '/^model:/,/^[a-z]/ s|^  provider: .*|  provider: "'"$safe_provider"'"|' "$cfg"
    if [ -n "$MODEL" ]; then
        safe_model="$(printf '%s' "$MODEL" | tr -cd 'a-zA-Z0-9_./:-')"
        sed -i '/^model:/,/^[a-z]/ s|^  default: .*|  default: "'"$safe_model"'"|' "$cfg"
    fi

    # Validate the edited YAML still parses (best effort, needs venv Python).
...
try:
    ...
    print('WARNING: config.yaml no longer parses:', e)
" || true
    fi
    log_success "Configured model.provider=${AI_PROVIDER}${MODEL:+ model.default=${MODEL}} in $cfg"
}
...
install_system_packages() {
    ...
    if command -v rg &> /dev/null; then
        ...
        HAS_RIPGREP=true
    elif [ "$INSTALL_RIPGREP" = "yes" ]; then
        need_ripgrep=true
    else
        log_info "Skipping ripgrep install (INSTALL_RIPGREP=$INSTALL_RIPGREP)"
    fi
    ...
    if command -v ffmpeg &> /dev/null; then
        ...
        HAS_FFMPEG=true
    elif [ "$INSTALL_FFMPEG" = "yes" ]; then
        need_ffmpeg=true
    else
        log_info "Skipping ffmpeg install (INSTALL_FFMPEG=$INSTALL_FFMPEG)"
    fi
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # [MODIFIED] sudo needs a password — never prompt (would block an
        # unattended/background run). Skip with a warning instead.
        elif command -v sudo &> /dev/null; then
            log_warn "sudo needs a password — skipping optional system packages (${pkgs[*]})"
...
install_deps() {
    ...
    # [MODIFIED] Controlled by INSTALL_BUILD_TOOLS in config.txt (default yes);
    # never prompts for a sudo password during unattended installs.
    if [ "$DISTRO" = "ubuntu" ] || [ "$DISTRO" = "debian" ]; then
        ...
        if [ "$need_build_tools" = true ] && [ "$INSTALL_BUILD_TOOLS" = "yes" ]; then
            ...
            if command -v sudo &> /dev/null; then
                if sudo -n true 2>/dev/null; then
                    ...
                    log_warn "sudo needs a password — skipping build tools (build-essential, python3-dev, libffi-dev)"
                    log_info "Install manually after setup: sudo apt-get install -y build-essential python3-dev libffi-dev"
                fi
            fi
        elif [ "$need_build_tools" = true ]; then
            log_info "Skipping build tools (INSTALL_BUILD_TOOLS=$INSTALL_BUILD_TOOLS)"
        fi
    fi
...
maybe_start_gateway() {
    ...
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        # [MODIFIED] Never run QR pairing during an unattended install — the
        # QR scan needs interaction and would block a background run. Pair
        # afterwards with 'hermes whatsapp'.
        log_info "WhatsApp is enabled but not yet paired."
        log_info "After install, run 'hermes whatsapp' to pair via QR code."
    fi
    ...
    echo ""
    # [MODIFIED] Unattended install — decision comes from config.txt
    # (INSTALL_GATEWAY, default yes), never an interactive prompt.
    ...
    case "$INSTALL_GATEWAY" in
        yes|true|1|y|Y)
            ...
            ;;
        *)
            log_info "Skipping gateway install (INSTALL_GATEWAY=$INSTALL_GATEWAY)"
            ;;
    esac
...
install_desktop() {
    ...
    if run_with_timeout "$DESKTOP_BUILD_TIMEOUT" bash -c 'cd "$1" && npm ci' _ "$INSTALL_DIR"; then
        ...
        log_warn "[CONTAINER] Desktop npm install failed — continuing without browser tools"
        # In container environment, desktop tools are not needed
        # Return success so container setup completes
        [ -n "${HERMES_CONTAINER:-}" ] && return 0 || return 1
    fi
